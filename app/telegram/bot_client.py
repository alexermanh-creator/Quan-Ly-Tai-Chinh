...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. ƯU TIÊN TUYỆT ĐỐI CHO MENU CHÍNH
    if text in ["🏠 Trang chủ", "💼 Tài sản của bạn", "🏠 Home", "⬅️ Back", "❌ Hủy"]:
        if 'dashboard' in modules:
            result = modules['dashboard'].run(user_id)
            await format_response(update, 'dashboard', result)
            return

    # 2. KIỂM TRA TÊN MODULE
    ...
    if 'transaction' in modules:
        ...
        if isinstance(res, str):
            # Nếu nhận tín hiệu HOME_SIGNAL hoặc thông báo thành công
            if res == "HOME_SIGNAL" or any(x in res for x in ["thành công", "Trang chủ", "chiết khấu"]):
                result = modules['dashboard'].run(user_id)
                if res != "HOME_SIGNAL": await update.message.reply_text(res)
                await format_response(update, 'dashboard', result)
            else:
...
async def format_response(update: Update, m_id: str, result: dict):
    ...
    if m_id == "dashboard":
    ...
    elif m_id == "stock" or m_id == "transaction":
        ...
        await update.message.reply_text(str(result), reply_markup=main_markup)
